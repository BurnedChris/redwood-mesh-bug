...
export const getCurrentUser = async (decoded, { _token, _type }) => {
  ...
  const metadata = await mAdmin.users.getMetadataByToken(decoded)

  if (!metadata.publicAddress || !metadata.issuer) {
    throw new AuthenticationError('Uh, oh') // error from magic as it should come back with address or issuer
  }

  return { ...metadata, roles: parseJWT({ decoded }).roles }
}
